...
public class Sendgrid {
    ...
    public Sendgrid(String username, String password) {
        ...
        try {
            setCategory("google_sendgrid_java_lib");
    ...
    public Sendgrid setBcc(String email) throws JSONException {
        ...
        if (_useHeaders() == true)
        {
            addFilterSetting("bcc", "enable", "1");
            addFilterSetting("bcc", "email", email);
        }
    ...
    public Sendgrid setCategories(String[] category_list) throws JSONException {
        ...
        addCategory("google_sendgrid_java_lib");
    ...
    protected Map<String, String> _prepMessageData() throws JSONException {
        ...
        if (_useHeaders() == true) {
            ...
            params.put("to", getFrom());
            JSONArray tos_json = new JSONArray(getTos());
            headers.put("to", tos_json);
            setHeaders(headers);
            params.put("x-smtpapi", _escapeUnicode(getHeaders().toString()));
        } else {
            params.put("to", getTos().toString());
            if (getToNames().size() > 0) {
    ...
    public void send() throws JSONException {
        send(new WarningListener() {
			...
			public void warning(String w, Throwable t) {
                Sendgrid.this.serverResponse = w;
            }
        });
    }
    ...
    public void send(WarningListener w) throws JSONException {
        ...
        while (paramIterator.hasNext()) {
            ...
            if (key.equals("to") && getTos().size() > 0) {
                if (_useHeaders() == true){
                    ...
                    requestParams.append(_arrayToUrlPart(getTos(), "to")+"&");
                }
            } else {
                if (key.equals("toname") && getToNames().size() > 0) {
                    requestParams.append(_arrayToUrlPart(getToNames(), "toname").substring(1)+"&");
                } else {
                    try {
        ...
        if (getBccs().size() > 0){
            request += "?" +_arrayToUrlPart(getBccs(), "bcc").substring(1);
        }
        try {
            ...
            if (connection.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                for (int i = 0; i < errorsObj.length(); i++) {
                    ...
                    this.serverResponse += errorsObj.get(i);
                }
                w.warning(this.serverResponse, null);
            }
    ...
    private Boolean _useHeaders() {
        if ((_preferNotToUseHeaders() == true) && (_isHeadersRequired() == false))
			return false;
		else
			return true;
    }
